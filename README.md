...
<b>24.03.07</b> 
- 오류 수정 
  - 글 작성자와 대댓글의 원댓글 작성자가 동일할 경우 글 작성자에게 새 댓글 알림이 발생하지 않고 새 대댓글 알림만 발생하도록 수정
...
- 기능 추가
  - 관리자 회원가입
  - 관리자에게 커뮤니티글 삭제/댓글 삭제/여행 데이터 다운로드 권한 부여
  - 여행 API 데이터 다운로드 성공시 홈 화면으로 리다이렉트

<b>24.03.12</b> 
- 기능 추가
  - 관리자 페이지
  - 이미지 일괄 삭제 기능 추가 후 해당 기능에 대해 관리자에게 권한 부여 
  - 로그인 폼에서 엔터키로 로그인 요청 가능하도록 연결
  - 관리자 관련 Swagger 추가 작성


- UI 수정
  - 마이페이지의 프로필과 비밀번호 변경 폼 넓이 수정
  - 기본 알림 모달을 sweet_alert2로 일괄 수정
  

- 오류 수정
  - 회원가입 뷰 리턴 URL 수정
  - 카카오 회원이 탈퇴할 경우 리포지토리에서 회원을 소셜 코드가 아닌 DB_ID로 찾도록 메서드의 파라미터를 수정
  - 여행 상세글의 대상 id가 유효하지 않을 경우 404 예외 화면을 반환하도록 수정
  - 여행 목록에서 검색 결과가 없을 경우 표시되는 화면 수정

<b>24.03.13</b> 
- 리팩토링
  - 의존관계 주입을 생성자 주입 방식으로 변경: config 파일에서는 필드 주입 혼용
  - 관리자 이미지 삭제 요청 URL 변경
...
- 오류 수정
  - 커뮤니티 목록에서 검색 결과가 없을 경우 표시되는 화면 수정


<b>24.03.15</b>
- 오류 수정
  - User의 소셜로그인 코드 컬럼 삭제
  - 전체 Entity의 nullable 속성 수정
  - 댓글 중복 작성 오류 수정
...
- 오류 수정
  - 댓글 작성 오류 재수정
    - 댓글 중복 작성 & 대댓글 내용 누락
    - 엔터키와 댓글/대댓글 작성 연결 제거
  - EC2 인스턴스 오류로 인한 인스턴스 재시작에 따른 IP 변경


<b>24.03.22</b>
- 오류 수정
  - 커뮤니티 첨부 이미지에 최대 넓이 제한을 부여하여 이미지 본래 크기대로 표시되지 않도록 수정
  - 로그인이 필요한 요청에서 로그인하지 않았을 경우 500 예외를 반환하지 않고 로그인 화면으로 리다이렉트되도록 수정
  - 마이페이지 작성글 목록에서 댓글단 글을 클릭할 경우 탭 클릭 효과가 내 프로필로 변경되지 않고 작성글 목록으로 고정되도록 수정
  - 커뮤니티 작성/수정폼에서 게시판 카테고리 이름을 잡담/모임으로 수정


- UI 변경 
  - 메인 홈페이지의 캐러셀과 기존 이미지를 제거하고 단일 이미지 배경으로 변경 & 헤더를 이미지에 맞게 변경
