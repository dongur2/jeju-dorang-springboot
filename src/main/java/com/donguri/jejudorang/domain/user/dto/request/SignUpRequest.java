...
public record SignUpRequest (
        ...
        @Schema(description = "아이디 (5자 이상 20자 이하의 영문자와 숫자 조합)", example = "testuser1", required = true)
        ...
        @Schema(description = "닉네임 (2자 이상 15자 이하의 특수문자와 이모티콘을 제외한 문자열)", example = "테스트유저", required = true)
        ...
        @Schema(description = "이메일 주소", example = "usertest@email.com", required = true)
        @Email(message = "이메일 형식으로 작성해주세요.")
        @NotBlank(message = "이메일을 입력해주세요.")
        String emailToSend,

        @Schema(description = "이메일 인증 여부", example = "true", required = true, defaultValue = "false")
        @AssertTrue(message = "이메일 인증이 필요합니다.")
        boolean isVerified,

        @Schema(description = "제주도랑 이용약관 동의 여부 (필수 동의)", example = "true", required = true, defaultValue = "false")
        @AssertTrue(message = "제주도랑 이용약관에 동의해야 가입이 가능합니다.")
        boolean agreeForUsage,

        @Schema(description = "필수 개인정보 수집 및 이용에 동의 여부 (필수 동의)", example = "true", required = true, defaultValue = "false")
        @AssertTrue(message = "필수 개인정보 수집 및 이용에 동의해야 가입이 가능합니다.")
        boolean agreeForPrivateNecessary,

        @Schema(description = "선택 개인정보 수집 및 이용에 동의 여부", example = "false", required = false, defaultValue = "false")
        boolean agreeForPrivateOptional,

        @Schema(description = "비밀번호 (8자 이상 20자 이하의 특수문자와 숫자가 적어도 하나가 포함된 문자열)", example = "testuser~11111", required = true)
        ...
        @Schema(description = "비밀번호 확인", example = "testuser~11111", required = true)
        @NotBlank(message = "비밀번호를 한 번 더 입력해주세요.")
        String passwordForCheck,

        @Schema(description = "권한 ('null'일 경우 USER, 'admin'일 경우 ADMIN)", example = "null", required = false)
        Set<String> role
