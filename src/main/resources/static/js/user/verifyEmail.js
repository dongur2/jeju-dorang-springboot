...
function sendMailForPwd() {
    ...
    if (mailSendForPwdRequest.email == null
        || mailSendForPwdRequest.email.trim() === "") {
        alert("이메일을 입력해주세요.");

    } else if (mailSendForPwdRequest.externalId == null
        || mailSendForPwdRequest.externalId.trim() === ""){
        alert("아이디를 입력해주세요.");

    } else {
        var xhr = new XMLHttpRequest();
        xhr.open("POST", "/user/email/find-pwd", true);
        xhr.setRequestHeader("Content-Type", "application/json");
